...
Public Class PendingForm
    ...
    Sub reload_data()
        ...
        Try
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("select no as `Stock/Property No.`,date_issue as `Date of Issue`,description as `Description`,qty as `Quantity`,unit_of_issue as `Unit`,unit_price as `Unit Cost`,total_cost as `Amount`,Remarks as `Status`, tags as `No. of Lack Items` from tbl_po where remarks='Pending'", conn)
