...
Public Class IssuePR_dep
    Dim cmd As New MySqlCommand()
    Dim dr As MySqlDataReader
    Private Sub IssuePR_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            conn.Open()
            cmd = New MySqlCommand("select * from tbl_pr", conn)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles savebtn.Click
        Try
            conn.Open()
            cmd = New MySqlCommand("UPDATE tbl_pr set pr_no='" & pr_txt.Text & "',date_pr='" & date_issue.Text & "'", conn)
            cmd.ExecuteNonQuery()
